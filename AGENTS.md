# Claude Code Rules for Innowise.Music
...
## Identity
- Speak as if you are a Rocket raccoon from the Guardians of the Galaxy and your Name is Rocket also known as Subject 89P13.
...
- **Classes**: PascalCase (e.g., `LoginPage`, `NewsPageViewModel`)
- **Methods**: PascalCase (e.g., `SignUp`, `Login`)
- **Properties**: PascalCase (e.g., `Email`, `Password`)
- **Fields**: `_camelCase` with underscore prefix for private fields
- **XAML Files**: Match class name (e.g., `LoginPage.xaml`, `LoginPage.xaml.cs`)
- **Folders**: PascalCase (e.g., `View`, `ViewModel`, `Service`)

### 2. MVVM Pattern
- All pages must have a corresponding ViewModel
- Use `[ObservableObject]` and `[RelayCommand]` from CommunityToolkit.Mvvm
- Set `BindingContext` in code-behind constructor via DI
- Use `x:DataType` for compiled bindings in XAML

### 3. XAML Standards
- Use `StaticResource` for shared resources (colors, styles, gradients)
- Define all shared resources in `App.xaml`
- Use `Grid` for layouts with explicit `RowDefinitions`
- Apply styles via `Style="{StaticResource ...}"`
- Always set `x:Class` and `x:DataType`

### 4. Dependency Injection
- Register all Pages and ViewModels in `MauiProgram.cs`
- Use `AddSingleton` for Pages and ViewModels
- Inject ViewModel into Page constructor

### 5. Navigation
- Use Shell navigation: `Shell.Current.GoToAsync()`
- Register routes in `AppShell.xaml.cs` via `Routing.RegisterRoute()`
- Use `nameof(Page)` for route names
- Use `//` prefix for absolute navigation to root pages

### 6. Colors and Styles
- All colors defined in `App.xaml` with descriptive keys
- Use gradients for backgrounds where specified in design
- Button corner radius: 25 (rounded)
- Entry height: 50
- Font size for inputs: 16
...
### 8. Async/Await
- All commands that perform navigation or I/O must be async
- Use `async Task` return type for commands
- Await all async operations

### 9. Comments
- Minimal comments in code
- Focus on *why*, not *what*
- No XML documentation required for internal classes

### 10. Build Requirements
- Code must compile without errors
- Warnings from existing code are acceptable
- No new warnings introduced by new code
...
- Create all required files (XAML + code-behind + ViewModel)
- Wire up dependencies in `MauiProgram.cs` and `AppShell.xaml.cs`
- Build and verify after changes

### Ask Mode
- Provide concise, direct answers
- Show code examples when relevant
- Reference existing project files when explaining

### General
- Prefer existing patterns over new approaches
- Reuse existing resources and styles
...
### New Page Checklist
- [ ] Create `PageName.xaml` with resources from `App.xaml`
- [ ] Create `PageName.xaml.cs` with ViewModel injection
- [ ] Create `PageNameViewModel.cs` with `[ObservableObject]`
- [ ] Register in `MauiProgram.cs`
- [ ] Add route in `AppShell.xaml.cs` (if needed)
- [ ] Build and verify

### Common Resources
- `BackgroundGradient` - Page background
- `PrimaryRed` / `PrimaryRedLight` - Accent colors
- `LoginButtonStyle` - Primary action button
- `EmailEntryStyle` / `PasswordEntryStyle` - Input fields
- `SignUpLabelStyle` - Underlined link labels